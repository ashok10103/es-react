...
import classes from '../Containers/Facets.css';
...
class Facets extends Component {
  ...
  render() {
    ...
    const status = newData.map((item)=>(
      item.aggregations["status.keyword"].buckets.map((status,index)=>(
      <div key={index}>
        <input type="checkbox"  />
        <span>{status.key}</span >
        <span className={classes.doc_count}>{status.doc_count}</span>
      </div>   
      ))      
    ))
    const process = newData.map((item)=>(
      item.aggregations["process.keyword"].buckets.map((process,index)=>(
        <div key={index}>
          <input type="checkbox"  />
          <span>{process.key}</span >
          <span className={classes.doc_count}>{process.doc_count}</span>
        </div>   
        ))      
    ))
    const subprocess = newData.map((item)=>(
      item.aggregations["subprocess.keyword"].buckets.map((subprocess,index)=>(
        <div key={index}>
          <input type="checkbox"  />
          <span>{subprocess.key}</span >
          <span className={classes.doc_count}>{subprocess.doc_count}</span>
        </div>   
        ))      
    ))
    const country = newData.map((item)=>(
      item.aggregations["country.keyword"].buckets.map((country,index)=>(
        <div key={index}>
          <input type="checkbox"  />
          <span>{country.key} </span ><span className={classes.doc_count}>{country.doc_count}</span>
        </div>   
        ))      
    ))  
 
    return (
      <div className={`container-fluid ${classes.facets_container}`}>

          <div className="row ">
            <div className={`col-xs-3 ${classes.facets}`}>
                Status
                <ul>
                  {status}
                </ul>
            ...
            <div className={`col-xs-9 ${classes.hits_container}`}>
              <Hits/>
            </div>
          </div>
      </div>
